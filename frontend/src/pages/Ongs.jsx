    ...
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          <div>
            <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">Conheça as ONGs</h1>
            <p className="mt-2 text-lg text-gray-600">Instituições sérias que precisam do seu apoio para continuar salvando vidas.</p>
          </div>
          <Link to="/ongs/indique" className="bg-white text-primary border border-primary/20 px-6 py-3 rounded-full font-medium hover:bg-primary/5 transition-colors flex items-center gap-2 whitespace-nowrap shadow-sm">
            Indique uma ONG
          </Link>
        </div>
        ...
        <form onSubmit={handleSearch} className="mb-12 relative max-w-2xl">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-32 py-4 rounded-full border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all shadow-sm text-lg"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-primary text-white px-6 py-2 rounded-full font-medium hover:bg-primary-dark transition-colors"
            >
              Buscar
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white p-12 rounded-3xl border border-gray-100 text-center">
            ...
            <button onClick={() => { setBuscaCidade(''); loadOngs(); }} className="text-primary hover:underline">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {ongs.map(ong => (
              <div key={ong.id} className="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
                {ong.imagem ? (
                  <img src={ong.imagem} alt={ong.nome} className="w-full h-48 object-cover" />
                ) : (
                  <div className="w-full h-48 bg-emerald-50 flex items-center justify-center text-emerald-300">
                    <span className="text-5xl font-bold">{ong.nome.charAt(0).toUpperCase()}</span>
                  </div>
                )}
                
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xl font-bold text-gray-900">{ong.nome}</h3>
                  </div>
